// tx telah membantu membuat script ini
// script by Mixuu
const {
...
exports.makeWASocket = (connectionOptions, options = {}) => {
    let conn = makeWALegacySocket || makeWASocket(connectionOptions);
            ...
            jid,
            {
                ...
                contextInfo: {
                    ...
                    externalAdReply: {
                        ...
                        body: body,
                        thumbnailUrl:
                            "https://telegra.ph/file/dc229854bebc5fe9ccf01.jpg",
...
`.trim();
        ...
        (conn.reply = (jid, text = "", quoted, options) => {
            return Buffer.isBuffer(text)
                ...
                : conn.sendMessage(
                      ...
                      { quoted, ...options, mentions: conn.parseMention(text) }
                  );
        });
    ...
    conn.pushMessage = async m => {
        ...
        for (const message of m) {
            try {
                // if (!(message instanceof proto.WebMessageInfo)) continue // https://github.com/Baileys/pull/696/commits/6a2cb5a4139d8eb0a75c4c4ea7ed52adc0aec20f
                if (!message) continue;
                if (
                ...
                if (message.message?.[mtype]?.contextInfo?.quotedMessage) {
                    ...
                    if (
                        ...
                        let qChatsMessages;
                        if (
                            (qChatsMessages = Object.entries(qChats.messages))
                                .length > 40
                        )
                ...
                if (
                    ...
                    let chatsMessages;
                    if (
                        (chatsMessages = Object.entries(chats.messages))
                            .length > 40
                    )
    ...
    if (m.message) {
        ...
        m.mtype =
            ...
            (mtype.length >= 3 &&
                ...
                mtype[1]) || // Sometimes message in midle if mtype length is greater than or equal to 3!
        ...
        if (m.mtype == "protocolMessage" && m.msg.key) {
            ...
            m.msg.key.fromMe =
                conn.decodeJid(m.msg.key.participant) ===
                conn.decodeJid(conn.user.id);
            if (
                !m.msg.key.fromMe &&
        ...
        if (m.quoted) {
            ...
            m.quoted.isBaileys =
                (m.quoted.id && m.quoted.id.length === 16) || false;
            ...
            m.getQuotedObj = m.getQuotedMessage = async () => {
                ...
                let q = M.fromObject(
                    (await conn.loadMessage(m.quoted.id)) || vM
                );
            ...
            m.quoted.copy = () =>
                exports.smsg(conn, M.fromObject(M.toObject(vM)));
