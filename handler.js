  ...
  handler: async function (chatUpdate) {
    ...
    try {
      ...
      try {
        ...
        let settings = global.db.data.settings[conn.decodeJid(this.user.id)];
      ...
      if (!m.fromMe && db.data.settings[conn.decodeJid(this.user.id)]?.self)
        return;
      ...
      const isROwner = global.owner
        ...
        .flatMap((number) => [number + "@lid", number + "@s.whatsapp.net"])
        .includes(m.sender);
      ...
      const bot =
        participants.find((u) => conn.decodeJid(u.id) == conn.user.lid) || {}; // Your Data
            ...
            if (e) {
              ...
              if (e.name)
                for (let [jid] of global.owner.filter(
                  ([number, isCreator, isDeveloper]) => isDeveloper && number,
                )) {
...
global.dfail = (type, m, conn) => {
  ...
  if (msgg)
    return conn.reply(m.chat, msgg, m, {
      contextInfo: {
